...
class Printer:
    ...
    def print_best_scores(self, results):
        with open(os.path.join(self._scores_path, f'best_scores.txt'), 'w') as f:
            f.write(tabulate.tabulate(
                results,
                headers=['Window size', 'Model', 'Params', 'Precision', 'Recall', 'F1 score'],
                tablefmt='orgtbl'
            ))
